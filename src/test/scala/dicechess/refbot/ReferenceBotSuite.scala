...
class ReferenceBotSuite extends munit.CatsEffectSuite:
  ...
  test("keepAlive reconnects after a normally ended stream without a live API"):
    for
      attempts <- Ref.of[IO, Int](0)
      pauses   <- Ref.of[IO, List[FiniteDuration]](Nil)
      stream = attempts.updateAndGet(_ + 1).map { n =>
        n == 1 // first stream ended normally; second stream proves the reconnect and stops the loop
      }
      _     <- ReferenceBot.keepAliveWith("test", delay => pauses.update(_ :+ delay), () => 0.5)(stream)
      count <- attempts.get
      seen  <- pauses.get
    yield
      assertEquals(count, 2)
      assertEquals(seen, List(ReferenceBot.ReconnectInitialDelay))

  test("keepAlive treats a revoked token as terminal"):
    val unauthorized = UnexpectedStatus(Status.Unauthorized, GET, Uri.unsafeFromString("http://localhost/stream"))
    for
      attempts <- Ref.of[IO, Int](0)
      result   <- ReferenceBot
        .keepAliveWith("test", _ => IO.unit, () => 0.5)(attempts.update(_ + 1) *> IO.raiseError(unauthorized))
        .attempt
      count <- attempts.get
    yield
      assertEquals(count, 1)
      assertEquals(result, Left(unauthorized))

  test("reconnect jitter is centered, bounded, and the exponential delay is capped"):
    assertEquals(ReferenceBot.reconnectDelay(10.seconds, 0.5), 10.seconds)
    assertEquals(ReferenceBot.reconnectDelay(10.seconds, 0.0), 8.seconds)
    assertEquals(ReferenceBot.reconnectDelay(10.seconds, 1.0), 12.seconds)
    assertEquals(
      ReferenceBot.reconnectDelay(ReferenceBot.ReconnectMaximumDelay, 1.0),
      ReferenceBot.ReconnectMaximumDelay
    )
    assertEquals(
      ReferenceBot.nextReconnectDelay(ReferenceBot.ReconnectMaximumDelay),
      ReferenceBot.ReconnectMaximumDelay
    )
  ...
  test("run fails immediately for an unauthorized account stream"):
    val unauthorizedClient = Client[IO](_ => Resource.pure(Response[IO](Status.Unauthorized)))
    Supervisor[IO].use: supervisor =>
      ReferenceBot(testConfig, unauthorizedClient, supervisor, NoOpStrategy).run.attempt.map:
        case Left(UnexpectedStatus(Status.Unauthorized, _, _)) => assert(true)
        case other => fail(s"expected unauthorized stream failure, got $other")
  ...
  test("run reconnects the account stream on a transient drop without killing in-flight games (#53)"):
    ...
    for
      ...
      // The side effect fires after the maximum 1.2s first reconnect delay (plus the 50ms drop), so it proves the
      // game remained alive after reconnection without racing the jittered retry.
      gameBody = Stream.sleep_[IO](1500.millis) ++ Stream.eval(order.update(_ :+ "game-g1-still-alive")).drain
      client   = fakeClient(order, accountBody, games, _ => gameBody)
      _ <- Supervisor[IO].use: supervisor =>
        val bot = ReferenceBot(testConfig, client, supervisor, NoOpStrategy)
        bot.run.start.flatMap(fiber => IO.sleep(2.seconds) *> fiber.cancel)
      recorded <- order.get
    yield
      val expected = List(
        "account-connected",
